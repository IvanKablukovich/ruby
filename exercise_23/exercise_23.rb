require "spec_helper" #required file

describe "Dog" do  #describe class

  describe ".new" do
    it "initializes with one argument - a name" do
      expect{Dog.new("Snoopy")}.to_not raise_error #check for null and exeptions
    end
  end

  let!(:pluto) { Dog.new("Pluto") } #dont have idea
  let!(:fido) { Dog.new("Fido") }
  let!(:maddy) { Dog.new("Maddy") }

  after(:each) do
    Dog.class_variable_set(:@@all, []) #class variables
  end

  describe "class variables" do
    describe "@@all" do
      it "has a class variable, @@all, the points to an array" do
        expect(Dog.class_variable_get(:@@all)).to be_a(Array) #set :@@all to array
      end
    end
  end
  ...
  describe "class methods" do
    describe ".all" do
      it "is a class method returns all dog instances" do
        expect(Dog.all).to match([pluto,fido,maddy])  #matches
      end
    end
